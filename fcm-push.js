...
(() => {
  ...
  // Moves this browser from the previous SPE ID's certificate topic to the new
  // one (an empty speId just leaves the previous topic).
  window.speBasrahSyncCertificateTopic = function(speId,previousSpeId) {
    const previous = syncJob || Promise.resolve();
    const run = previous.catch(()=>{}).then(async()=>{
      if (disabled || Notification.permission !== 'granted') return false;
      const token = await deviceToken();
      await jsonFetch('/fcm-web-cert-topic',{token,speId,previousSpeId});
      return true;
    });
    syncJob = run;
    return run;
